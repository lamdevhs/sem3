...
from Matrix import (idMatrix, nullVector, firstNonNullEachLine, negate, unitVector,
    familyFromMatrix, columnAt, isNullVector, ncols)
...
# of the kernel of A, or is an empty list if the kernel is reduced to
# the null vector.
...
#     Solution n = (Family n, Vector n)
# that is, a solution is the couple composed of
# the kernel basis, or [] if the kernel is {0}
...
# of the left side matrix, minus the columns of the
# pivots, and we get the basis of the kernel.
#
# the particular solution is just the right side vector.
# 
# we thereafter return the tuple of both of them together,
...
def extractSolution(lines):
    ...
    p = ncols(leftMatrix)
    maybePivotalColIs = firstNonNullEachLine(leftMatrix)
    if maybePivotalColIs == Nothing:
        error(extractSolution, "one pivotal line is full of zeroes")
    pivotalColIs = maybePivotalColIs.justValue()
    nonPivotalColIs = subtractLists(range(p), pivotalColIs)
    leftMatrix = negate(leftMatrix)
    for index in nonPivotalColIs:
        uv = unitVector(p, index)
        leftMatrix.insert(index, uv)
        rightVector.insert(index, 0)
    #print "filled"
    #printMatrix(leftMatrix)
    kernelBasis = filterOutIx(pivotalColIs, familyFromMatrix(leftMatrix))
